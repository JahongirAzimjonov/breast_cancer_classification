...
def display_pearson_correlation_coefficients(corr):
    ...
    plt.yticks(fontsize=11, fontweight="bold")
    plt.savefig(path_to_save_plots + "Correlation Coefficients.png", dpi = 100)
    plt.show()
    ...
    for row in range(num_of_subplot_rows):
        ...
        for col in range(num_of_subplot_cols):
            ...
            if colID < len(X_wsf_cols):
                ...
                sns.distplot(X_with_selected_features[X_wsf_cols[colID]][result[target_name]==1], color='r', label = 'Malignant', ax=axes[row,col])
                axes[row,col].legend()
                colID += 1
    fig.suptitle("The Features, Selected based on the " + feature_selection_method_name, fontsize = 15, fontweight="bold")
    fig.tight_layout()
    fig.subplots_adjust(top=0.8)
    plt.savefig(path_to_save_plots + feature_selection_method_name + ".png", dpi = 100)
    plt.show()
...
def app():
    global path_to_save_docs
    global path_to_save_plots
    file_path = get_file_path('./')
    file_name, file_ext = get_file_name_and_ext(file_path)

    # Read data from csv to the pandas dataframe
    data = pd.read_csv(file_path)
    print(data.head())

    plot_dir = "Results/" + file_name + "/" + path_to_save_plots
    if not os.path.exists(plot_dir):
        os.makedirs(plot_dir)
    doc_dir = "Results/" + file_name + "/" + path_to_save_docs
    ...
    # result[target_name] = data.iloc[:,1]
    target_output[target_name] = data[target_name]
    print(f"target_output[target_name]: \n{target_output[target_name]}\n")
    
    # Dataset target output class distribution imbalance test
    dataset_text = f"Dataset Class Distribution Imbalance Test for\nthe {file_name} Dataset after Dropping Duplicate Records"
    dataset_imbalance_test(target_output=data[target_name], dataset_text=dataset_text) 

    # data = data.iloc[:,1:-1]
    # print(f"data.iloc[:,1:-1]: \n{datafig1 = plt.figure(figsize=(20,10), layout="constrained").iloc[:,1:-1]}\n")     
    data = data.drop(["id"], axis=1)
    print(f"data.drop(['id']): \n{data}\n")     

    label_encoder = LabelEncoder()              
    # data.iloc[:,0] = label_encoder.fit_transform(data.iloc[:,0]).astype('float64')    
    # print(f"data.iloc[:,0]: \n{data.iloc[:,0]}\n")
    data[target_name] = label_encoder.fit_transform(data[target_name]).astype('float64')
    print(f"data[target_name]: \n{data[target_name]}\n")

    x_train, x_test, y_train, y_test = train_test_split(data.values, data[target_name].values, test_size = 0.3, random_state=42)  
    ...
    corr = pearson_correlation_coefficients(data)   
    # corr_row_number = 40
    ...
    data_modeled, selected_feature_names_with_corr_coef = feature_selection_with_pearson_corr_coef(df = data, 
